...
void initializeFlowers() {
	...
	for(int i = 0; i < 20; ++i) { // Create 20 static flowers
		...
		f.y = frand(worldB + 0.1f, worldB + 0.2f); // Place them on the foreground grass
...
void startGame(){
	...
	// Flowers are initialized only once in main, but this ensures they exist if main wasn't properly run.
	if (flowers.empty()) {
		initializeFlowers();
	}
...
Falling makeObj(ObjType t){
	...
	f.radius = (t==ObjType::GoldenEgg? 0.045f : t==ObjType::Bomb? 0.032f : 0.038f);
...
ObjType getRandomObjType() {
	...
	if (r < 30) return ObjType::Bomb; // 15% chance
...
void drawDryGrassStack(float x, float y, float scale) {
	...
	glLineWidth(1.5f);
	glBegin(GL_LINES);
	glVertex2f(-0.06f, 0.08f); glVertex2f(0.0f, 0.18f);
	glVertex2f( 0.05f, 0.03f); glVertex2f(0.07f, 0.14f);
	glEnd();
...
void drawGradientBG(){
	...
	drawRect(houseX + houseW * 0.4f, houseY - houseH * 0.5f, houseW * 0.25f, houseH * 0.5f);
	// Window
	glColor3f(0.6f, 0.8f, 1.0f);
	drawRect(houseX - houseW * 0.4f, houseY + houseH * 0.2f, houseW * 0.3f, houseH * 0.3f);

	// Draw Dry Grass Stacks (New Element)
	drawDryGrassStack(-0.7f, worldB + 0.3f, 1.0f); // Near left edge, on the distant ground
	drawDryGrassStack(0.3f, worldB + 0.3f, 0.8f);  // Near center

	// Foreground Grass (Color change: vibrant green)
	glColor3f(0.2f,0.65f,0.3f); drawRect(0,-0.94f,1.0f,0.14f);
...
void drawChicken(){
	...
	glColor3f(0.95f,0.95f,0.95f); drawCircle(0,0,0.065f,36); // Body
	glColor3f(1.0f,1.0f,1.0f); drawCircle(-0.015f,-0.01f,0.045f,28); // Lower body
	glColor3f(0.9f,0.9f,0.9f); drawCircle(0.055f,0.05f,0.037f,28); // Head
...
void drawBasket(){
	...
	// Basket color
	glColor3f(0.65f,0.35f,0.15f); drawRect(0, 0.0f, basket.halfW, basket.h*0.85f);

	glColor3f(0.5f,0.25f,0.1f); glLineWidth(2); // Darker lines
	glBegin(GL_LINES);
...
void drawObj(const Falling& o){
	...
	if(o.type==ObjType::Bomb) bombShape(o);
	else eggShape(o);
	glPopMatrix();
}
...
void display(){
	...
	glClearColor(0.98f,0.98f,0.95f,1); glClear(GL_COLOR_BUFFER_BIT);
	glLoadIdentity();
	...
	if(screenState==Screen::Menu){
		...
		drawText(-0.95f,0.58f,"- Eggs: +1 (Normal), +5 (Blue), +10 (Golden).");
		drawText(-0.95f,0.48f,"- Bomb: Lose 1 Life (Game Over at "+std::to_string(MAX_BOMBS_ALLOWED)+" Bombs)");
		drawText(-0.95f,0.38f,"- Space: Pause/Resume, Enter: Select, Esc: Back");
		...
		glColor3f(0.5f,0.35f,0.15f); drawRect(0,0.65f,0.92f,0.018f);
		chicken.y = 0.70f;
		drawChicken();
		...
		if (timeLeft <= 0) {
			...
			gameOverMsg = "Caught " + std::to_string(MAX_BOMBS_ALLOWED) + " Bombs!";
		} else {
			 gameOverMsg = "Game Over!";
		}
		drawText(-0.32f,0.02f, gameOverMsg);
		drawText(-0.32f,-0.08f,"Final Score: "+std::to_string(score)+" Best: "+std::to_string(highScore));
